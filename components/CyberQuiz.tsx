...
export const CyberQuiz: React.FC<Props> = ({ onComplete, completedQuestions }) => {
    ...
    if (activeQuestion !== null) {
        ...
        return (
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 animate-in fade-in zoom-in-95 duration-300">
                <div className="flex justify-between items-center mb-6">
                    <span className="bg-indigo-900/50 text-indigo-300 text-xs font-mono px-2 py-1 rounded">SORU #{q.id}</span>
                    <span className="text-yellow-400 font-bold text-sm">Ödül: {q.xpReward} XP</span>
                </div>
                
                <h3 className="text-xl text-white font-bold mb-6">{q.question}</h3>
                ...
                {isAnswered && (
                    <div className={`mb-6 p-4 rounded-lg ${feedback === 'correct' ? 'bg-green-900/20 text-green-200' : 'bg-red-900/20 text-red-200'}`}>
                ...
                <div className="flex gap-3">
                    <button onClick={reset} className="flex-1 py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-xl font-semibold">
                        Kapat
                    </button>
                    {!isAnswered && (
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="col-span-full bg-gradient-to-r from-indigo-900/50 to-purple-900/50 p-6 rounded-2xl border border-indigo-500/30 mb-4 flex items-center justify-between">
                <div>
                    <h3 className="text-xl font-bold text-white mb-1">Siber Güvenlik Arenası</h3>
                    <p className="text-indigo-200 text-sm">Bilgini test et, seviye atla ve rozetleri topla.</p>
                </div>
                <GamepadIcon className="w-12 h-12 text-indigo-400 opacity-50" />
            </div>
                ...
                return (
                    <button
                        ...
                        className={`relative p-6 rounded-xl border text-left transition-all group ${
                            isDone 
                            ? 'bg-slate-900 opacity-60 border-slate-800 cursor-default' 
                    ...
                    >
                        ...
                        <h4 className={`font-semibold mb-4 ${isDone ? 'text-slate-500' : 'text-slate-200 group-hover:text-white'}`}>
                            {q.question.substring(0, 50)}...
                        </h4>
                        <div className="flex items-center gap-2">
                            <div className={`text-xs font-bold px-2 py-1 rounded ${isDone ? 'bg-slate-800 text-slate-600' : 'bg-yellow-900/30 text-yellow-400'}`}>
                                +{q.xpReward} XP
                            </div>
                            {isDone ? <span className="text-xs text-green-500 font-medium">Tamamlandı</span> : <span className="text-xs text-indigo-400 font-medium group-hover:underline">Başla &rarr;</span>}
                        </div>
                    </button>
                )
            })}
